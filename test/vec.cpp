...
auto main() -> int{
  ...
  test("vec default init", []{
    return m::vec4() == m::vec4(0.0) && m::vec4() == m::vec4(0.0, 0.0, 0.0, 0.0);
  });

  test("vec single param ctor", []{
    auto vec = m::vec4(1234.0);
    return 
      vec.x == 1234.0 &&
      vec.y == vec.x &&
      vec.z == vec.x &&
  ...
  test("vec * scalar", [&]{
    return vec2 * -2.0 == m::vec4(-4.0, -6.0, -10.0, -14.0);
  });
  ...
  test("normalization", []{
    auto vec = m::vec4(1.0);
    return vec.normalized() == vec / vec.len();
  });
 
  test("unit vector len", [&]{
    return vec.normalized().len() == 1.0;
  });
